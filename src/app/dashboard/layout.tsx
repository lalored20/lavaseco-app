...
import ShiftProtection from '@/components/auth/ShiftProtection';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    if (isCEO) {
        navItems.push({
            name: 'Configuración',
            href: '/dashboard/settings', // Parent href acts as ID for toggle
            icon: Settings,
            children: [
                { name: 'Administradores', href: '/dashboard/settings' }
            ]
        });
    }
                    ...
                    <aside className="w-64 bg-white/80 backdrop-blur-xl border-r border-orchid-200 hidden md:flex flex-col z-20 shadow-xl shadow-orchid-500/5">
                        <div className="p-6 flex items-center gap-3">
                            <div className={clsx(
                                ...
                                isCEO
                                    ...
                                    : "bg-gradient-to-br from-orchid-500 to-orchid-700 shadow-orchid-500/30"
                            )}>
                                <LayoutDashboard size={20} />
                            </div>
                            <div>
                                <h1 className="font-bold text-lg tracking-tight text-slate-900">
                                    {isCEO ? "Lavaseco Orquídeas - CEO" : "Lavaseco"}
                                </h1>
                        ...
                        <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                            {navItems.map((item) => {
                                ...
                                if (hasChildren) {
                                    isActiveParent = pathname.startsWith(item.href);
                                } else {
                                    isActiveParent = pathname === item.href || (pathname.startsWith(`${item.href}/`) && !navItems.some((n) => n.href !== item.href && pathname.startsWith(n.href) && n.href.length > item.href.length));
                                }
